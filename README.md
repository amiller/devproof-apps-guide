...
Best practices and pitfalls for building devproof apps on dstack/Phala Cloud.

**See also:** [dstack-tutorial](https://github.com/aspect-build/dstack-tutorial) (getting started) · [Phala Cloud docs](https://docs.phala.com) · [dstack SDK](https://github.com/aspect-build/dstack) · [devproof-audits-guide](https://github.com/aspect-build/devproof-audits-guide) (audit practices)

---

## Deployment Notes & Pitfalls

> For people who've done the dstack tutorial and are now shipping something real.
> This is the stuff that wastes your afternoon.
...
──────────                   ──────────
npm run dev / docker compose Phala CVM (Intel TDX)
MemoryStorage / SQLite       Firestore / Neon Postgres
localhost:3000               <app-id>-3000.dstack-pha-prodN.phala.network
                             Custom domain via dstack-ingress (optional)
                             Vercel for frontend (optional)
```

Your app is a Node/Python server in a Docker container. Locally it runs with in-memory or SQLite storage. In production it runs in a Phala CVM with real KMS, attestation, and managed DB.
...
See `toy-example/verify.sh` for a working implementation.
...
## 11. Local Dev Setup Pattern

Two docker-compose files: one for local, one for deploy.
...
services:
  app:
    ...
    volumes:
      ...
      - app_data:/data          # SQLite persistence
    environment:
...
services:
  app:
    image: ghcr.io/you/app@sha256:...   # pinned digest
    ports:
      - "3000:3000"
    environment:
      - NODE_ENV=production
      - DATABASE_URL=${DATABASE_URL}     # if using managed DB

  # Optional: custom domain
  dstack-ingress:
    image: phalanetwork/dstack-ingress@sha256:...
    environment:
      - DOMAIN=app.yourdomain.com
      - GATEWAY_DOMAIN=_.dstack-pha-prodN.phala.network
    volumes:
...
**Storage abstraction pattern (from hermes):**
```typescript
interface Storage {
...
// Production: Firestore/Neon/Postgres
class ProductionStorage implements Storage { ... }

// Pick based on env
const storage = process.env.DATABASE_URL
  ? new ProductionStorage(process.env.DATABASE_URL)
  : new MemoryStorage()
```
...
- **[starter-kit-app/](./starter-kit-app/)** — Companion hello-world that integration-tests each section of this guide
- [Phala Cloud Networking Specs](https://docs.phala.com/phala-cloud/networking/specifications) — SSH access, gateway patterns
- [dstack-tutorial](https://github.com/aspect-build/dstack-tutorial) — 8-part progressive guide
- [devproof-audits-guide](https://github.com/aspect-build/devproof-audits-guide) — Trust model verification + 9 case studies
- [hermes](https://github.com/aspect-build/hermes) — Clean reference implementation of this pattern
- [ERC-733 Security Stages](https://github.com/aspect-build/devproof-audits-guide/blob/main/references/erc733-summary.md) — Stage 0 (ruggable) → Stage 3 (trustless)
